...
import { DAYS, DAY_LABELS, SERVICES, serviceLabel } from '@/lib/constants'
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      <div>
        ...
        <ImageUpload
          ...
          label="Profile Photo"
        />
        <p className="mt-1 text-xs text-muted">Shown on your public sitter profile. Optional — your account photo is used by default.</p>
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full rounded-xl border px-4 py-2.5 text-sm outline-none focus:ring-2 resize-none border-border bg-background"
        />
        <p className="mt-1 text-xs text-right text-muted">{bio.length}/500</p>
      </div>
      ...
      <div>
        <label className="mb-2 block text-sm font-medium text-foreground">Services Offered</label>
        <div className="flex flex-wrap gap-2">
          {SERVICES.map((s) => (
            <button
              ...
              className={`rounded-full px-4 py-1.5 text-sm font-medium transition-all border ${services.includes(s) ? 'border-primary bg-primary text-white' : 'border-border bg-background text-foreground'}`}
            >
              {serviceLabel(s)}
            </button>
          ))}
      ...
      <div>
        <label className="mb-3 block text-sm font-medium text-foreground">Weekly Availability</label>
        <div className="space-y-3">
          {DAYS.map((day) => {
            const d = availability[day] || { available: false, from: '09:00', to: '17:00' }
            return (
              <div key={day} className={`flex items-center gap-4 rounded-xl border border-border px-4 py-3 ${d.available ? 'bg-violet-50' : 'bg-gray-50'}`}>
                <button
                  type="button"
                  onClick={() => updateDay(day, 'available', !d.available)}
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 rounded-full transition-colors ${d.available ? 'bg-primary' : 'bg-gray-300'}`}
                  aria-label={`Toggle ${DAY_LABELS[day]}`}
                >
                  <span
                    className={`mt-0.5 inline-block h-5 w-5 rounded-full bg-white shadow transition-transform ${d.available ? 'translate-x-[22px]' : 'translate-x-0.5'}`}
                  />
                </button>
                ...
                {d.available ? (
                  <div className="flex items-center gap-2 text-sm text-muted">
                    <input
                      ...
                      className="rounded-lg border px-2 py-1 text-sm outline-none focus:ring-2 border-border bg-background text-foreground"
                    />
                    <span>to</span>
                    <input
                      type="time"
